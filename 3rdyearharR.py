    ...
    for j in range(len(lags)):
        ...
        if j>0:
            for k in range(lags[j]-lags[j-1]):
                temp=temp+np.roll(np.array(data[:]),(lags[j]-k+1))
...
def dataEndGen(data,target):
    dataEnd=np.roll(np.array(data[:]),-1*target)
    ...
    for i in range(depth):
       ...
       #Somehow recursively build tree? 
        ...
        for j in range(len(valid)):
            mse=HARmse(data[loc-MaxLags:window+loc+val+1+target],target,valid[j],window,val)
            mses.append(mse)
...
for i in range(len(targets)):
    ...
    Finals[3*i+2]=MSE(actual,preds)*10000#making results visible
s4="R/"+curr+"Finals.txt"
